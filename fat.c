...
#include "types.h"         // global AVRLIB defines
...
unsigned char fatInit()
{
	...
	printf("SectorsTotal: %lu\r\n", SectorsTotal);
	SectorsPerCluster	= bpb->bpbSecPerClust;
	BytesPerSector		= bpb->bpbBytesPerSec;
	FirstFATSector		= bpb->bpbResSectors + FirstDataSector;

	//update to sector after FAT(s) and root dir
...
fat_read_from_last_entry:
		...
		else if((unsigned char) de->deName[0] == SLOT_DELETED)	//0xE5
		{
...
unsigned short fatFindFreeAllocUnit( u32 * clusterNo, u32 * sect )
{
	...
	for( ; fatSect < (FirstFATSector + FATSectors); ++fatSect ) {
		...
		for( i = clst % fatEntriesPerSector; i < fatEntriesPerSector; ++i ) {
			...
			p = (u32*) &mmc_sector_buffer[i*fatSize];
			if( (*p & fatMask) == 0 ) {
				*clusterNo = clust + i;
				//printf("\nnext free cluster: %x\n", *clusterNo);
				*sect = fatSect;
...
u32 fatFileCreate (char *fileName, u32 fileSize) {
	...
	de = (struct direntry*) mmc_sector_buffer;
	...
	// update FAT
	NrOfClusters = fileSize/BytesPerSector/SectorsPerCluster;
